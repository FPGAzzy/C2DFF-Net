...
class CPCA(nn.Module):
    def __init__(self, channels, out_channels, channelAttention_reduce=4):
        ...
        self.dconv1_7 = nn.Conv2d(channels, channels, kernel_size=(1, 7), padding=(0, 3), groups=channels)#groups=channels 表示每个输入通道独立进行卷积，而不是将所有通道一起卷积。代表深度可分离
...
class PolarizedSelfAttention(nn.Module):  ###########################串联 QV互乘#############################################
    ...
    def forward(self, x):
        ...
        # 用QV相乘交互
        channel_wq_rgb = self.softmax_channel(channel_wq_rgb)  # (1，1024，1)
        channel_wq_ir = self.softmax_channel(channel_wq_ir)  # (1，1024，1)
        channel_wz_rgb = torch.matmul(channel_wv_rgb, channel_wq_ir).unsqueeze(
            -1)  # bs,c//2,1,1 （1,32,1024）*(1，1024，1)=（1，32，1，1）
        channel_wz_ir = torch.matmul(channel_wv_ir, channel_wq_rgb).unsqueeze(-1)  # bs,c//2,1,1 （1，32，1，1）
        channel_weight_rgb = self.sigmoid(
            self.ln(self.ch_wz(channel_wz_rgb).reshape(b, c, 1).permute(0, 2, 1))).permute(0, 2, 1).reshape(b, c, 1,
                                                                                                            1)  # bs,c,1,1（1，64，1，1）
        channel_weight_ir = self.sigmoid(self.ln(self.ch_wz(channel_wz_ir).reshape(b, c, 1).permute(0, 2, 1))).permute(
            0, 2, 1).reshape(b, c, 1, 1)  # bs,c,1,1 （1，64，1，1）
        ########################################################################################
        out_rgb = channel_weight_rgb * rgb  # （1，64，1，1）*（1，64，32，32）=（1，64，32，32）
        out_ir = channel_weight_ir * ir
        channel_out_rgb = torch.add(out_ir, rgb)  # （1，64，32，32）
        channel_out_ir = torch.add(out_rgb, ir)
        # channel_out_rgb=channel_weight_rgb * rgb #（1，64，1，1）*（1，64，32，32）=（1，64，32，32）
        # channel_out_ir = channel_weight_ir * ir

        # Spatial-only Self-Attention 串行
        spatial_wv_rgb = self.sp_wv(channel_out_rgb)  # bs,c//2,h,w（1，32，32，32）
        spatial_wq_rgb = self.sp_wq(channel_out_rgb)  # bs,c//2,h,w（1，32，32，32）
        spatial_wq_rgb = self.agp(spatial_wq_rgb)  # bs,c//2,1,1 （1，32，1，1）
        spatial_wv_rgb = spatial_wv_rgb.reshape(b, c // 2, -1)  # bs,c//2,h*w （1，32，1024）
        spatial_wq_rgb = spatial_wq_rgb.permute(0, 2, 3, 1).reshape(b, 1, c // 2)  # bs,1,c//2 （1，1，32）

        spatial_wv_ir = self.sp_wv(channel_out_ir)  # bs,c//2,h,w（1，32，32，32）
        spatial_wq_ir = self.sp_wq(channel_out_ir)  # bs,c//2,h,w
        spatial_wq_ir = self.agp(spatial_wq_ir)  # bs,c//2,1,1
        spatial_wv_ir = spatial_wv_ir.reshape(b, c // 2, -1)  # bs,c//2,h*w
        spatial_wq_ir = spatial_wq_ir.permute(0, 2, 3, 1).reshape(b, 1, c // 2)  # bs,1,c//2 （1，1，32）



        spatial_wq_rgb = self.softmax_spatial(spatial_wq_rgb)  # (1,1,32)
        spatial_wq_ir = self.softmax_spatial(spatial_wq_ir)  # (1,1,32)
        spatial_wz_rgb = torch.matmul(spatial_wq_ir,
                                      spatial_wv_rgb)  # bs,1,h*w #(1,1,32)mul()（1，32，1024）=(1,1,1024)
        spatial_weight_rgb = self.sigmoid(spatial_wz_rgb.reshape(b, 1, h, w))  # bs,1,h,w reshape后为（1，1，32，32）
        out_rgb = spatial_weight_rgb * channel_out_rgb  # （1，1，32，32）*(1，64，32，32）=(1,64,32,32)

        spatial_wz_ir = torch.matmul(spatial_wq_rgb, spatial_wv_ir)  # bs,1,h*w
        spatial_weight_ir = self.sigmoid(spatial_wz_ir.reshape(b, 1, h, w))  # bs,1,h,w
        out_ir = spatial_weight_ir * channel_out_ir
        # out = torch.cat((out_rgb, out_ir), 1)  # (1,128,32,32)
        spatial_out_rgb = torch.add(out_ir, channel_out_rgb)
        spatial_out_ir = torch.add(out_rgb, channel_out_ir)  # (1,64,32,32)
        # out=spatial_out_rgb+spatial_out_ir
        out = torch.cat((spatial_out_rgb, spatial_out_ir), 1)  # (1,128,32,32)
        out = self.conv(out)   # 为了不改变通道数，用卷积通道数减半 (1,64,32,32)
...
###################### PR ICAFUSION     TransformerFusionBlock ###############################
